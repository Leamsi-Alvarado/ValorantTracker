...
export const UserRankScreen = () => {
    const [user, onChangeUser] = useState("");
    const [tag, onChangeTag] = useState("");
    const [username, setUsername] = useState("");
    const [userrank, setUserrank] = useState("");
    const [usertag, setUsertag] = useState("");
    const [userelo, setAccountelo] = useState("")
    ...
    const obtener = () => {
        ...
        let newtag = "/" + tag
        let linkaccount = "https://api.henrikdev.xyz/valorant/v1/mmr/na/" + newname + newtag
        fetch(linkaccount)
            .then((response) => response.json()) //2
            .then((user) => {
                setUsername("username: " + user.data.name)
                setUsertag("Usertag: " + user.data.tag)
                setUserImage(user.data.images.small)
                setUserrank(user.data.currenttierpatched)
                setAccountelo("Elo: " + user.data.elo)
            });
